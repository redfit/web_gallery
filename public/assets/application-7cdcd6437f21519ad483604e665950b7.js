...
(function() {
  ...
  ready = function() {
    $('a[id*=mail]').tooltip();
    $(document).on("click", 'a[id*="mail"]', function() {
      var list;
      list = $(this).parents('li');
      return $('#mail_form').attr('action', list.data('url') + "/send_mail");
    }).on("click", 'a[id*="modal"]', function() {
      var list;
      list = $(this).parents('li');
      $('#form').attr('action', list.data('url')).attr('method', 'put');
      $('#bookmark_title').val(list.data('title'));
      return $('#bookmark_tag_list').val(list.data('tags'));
    });
    $(document).on("click", '#submit-dialog', function() {
      return $("#form").submit();
    });
    ...
    return $(document).on("ajax:success", 'a[id*="delete"]', function() {
      return $("#bookmarks").load("/bookmarks #bookmarks");
    });
  };
...
(function() {
  ...
  ready = function() {
    var canvas, ctx, img, popoverOptions;
    ...
    img.onload = function() {
      ...
      return canvas.onclick = function(evt) {
        var a, b, g, i, mouseX, mouseY, offset, r;
        ...
        return $.ajax({
          ...
          return $(html).popover(popoverOptions).hide().appendTo("#pick-list").fadeIn();
        });
      };
    };
    $(document).on("click", ".pick .icon-remove", function() {
      return $(this).parents("li.pick").fadeOut(function() {
        return $(this).remove();
      });
    }).on("click", ".pick", function() {
      var icon, offset, x, y;
      offset = $("canvas").position();
      x = parseInt($(this).data("x"), 10) - 14;
      y = parseInt($(this).data("y"), 10) - 7;
      icon = $("<i/>").addClass("icon-hand-right").css("position", "absolute").css("top", "" + (y + offset.top) + "px").css("left", "" + (x + offset.left) + "px").css("margin", "0").css("padding", "0");
      $(".icon-hand-right").remove();
      icon.appendTo($("#picker").parent());
      return icon.effect("bounce", {
        direction: 'left',
        distance: 30,
        times: 5
      }, 500);
    });
    return $(".pick").popover(popoverOptions);
  };
